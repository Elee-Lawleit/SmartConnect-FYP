...
const CreatePost = () => {
  ...
  const handleImageUploads = async (event: ChangeEvent<HTMLInputElement>) => {
    if (event.target.files) {
      ...
      setFileChecksum((prev) => [...prev, checksum])
    }
  }
        ...
        <div className="bg-gray-100 mb-3">
          <div className="bg-white p-8 rounded-lg shadow-md max-w-lg">
                      ...
                      <div key={index} className="shrink-0 relative">
                        ...
                        >
                          <Trash2
                            ...
                            onClick={() => selectedFiles.splice(index, 1)}
                          />
                        </button>
                      </div>
                    ))}
            ...
            <div className="mt-2 flex w-full justify-between">
              ...
              <Button size="sm" onClick={uploadPost} disabled={loading || isLoading}>
                {loading || isLoading? <Loader2 className="animate-spin"/> : "Create Post"}
              </Button>
            </div>
          </div>
        </div>
